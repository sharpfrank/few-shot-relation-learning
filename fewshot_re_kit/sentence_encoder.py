...
from transformers import GPT2Tokenizer, GPT2Model, GPT2DoubleHeadsModel, GPT2LMHeadModel
...
class GPT2SentenceEncoder(nn.Module):

    def __init__(self, pretrain_path, max_length, cat_entity_rep=False):
        nn.Module.__init__(self)
        self.roberta = GPT2Model.from_pretrained(pretrain_path)
        self.max_length = max_length
    ...
    def tokenize_v0(self, raw_tokens, pos_head, pos_tail):
        def getIns(bped, bpeTokens, tokens, L):
            resL = 0
            tkL = " ".join(tokens[:L])
            bped_tkL = " ".join(self.tokenizer.tokenize(tkL))
    ...
    def tokenize(self, raw_tokens, pos_head, pos_tail):
        ...
        tokens = []
        for ix, rt in enumerate(raw_tokens):
            token_list = self.tokenizer.tokenize(' ' + rt)
            tokens += token_list
            if ix == pos_head[-1]:
                pos1 = len(tokens) - 1
            elif ix == pos_tail[-1]:
...
class GPT2XSentenceEncoder(nn.Module):

    def __init__(self, pretrain_path, max_length, cat_entity_rep=False, mask_entity=False): 
        nn.Module.__init__(self)
        self.gpt2 = GPT2Model.from_pretrained(pretrain_path, return_dict=True)
        #self.gpt2 = GPT2LMHeadModel.from_pretrained(pretrain_path, return_dict=True)
        self.max_length = max_length
        self.tokenizer = GPT2Tokenizer.from_pretrained('gpt2')
        #new_tokens = {'cls_token': '[CLS]', 'entity_1_start': 'makeupword_e1s', 'entity_1_end': '<e1e>',
        #              'entity_2_start': '<e2s>', 'entity_2_end': '<e2e>', 'unused': '<unused>'}
        new_tokens = {'cls_token': '[CLS]'}
        num_added_tokens = self.tokenizer.add_special_tokens(new_tokens)
        embedding_layer = self.gpt2.resize_token_embeddings(len(self.tokenizer))
        self.cat_entity_rep = cat_entity_rep
        self.mask_entity = mask_entity

    def forward(self, inputs):
        if not self.cat_entity_rep:
            x = self.gpt2(inputs['word'], attention_mask=inputs['mask']).last_hidden_state
            iend = inputs['mask'].sum(dim=1) - 1
            y = torch.zeros((x.shape[0], x.shape[2]), dtype=x.dtype, device=x.device)
            for i in range(x.shape[0]):
                y[i] = x[i, iend[i]]
            # y = torch.mean(x, dim=1)
            #tensor_range = torch.arange(inputs['word'].size()[0])
            #y = x[tensor_range, inputs["pos1"]]
            return y
        else:
            outputs = self.gpt2(inputs['word'], attention_mask=inputs['mask']).last_hidden_state
            tensor_range = torch.arange(inputs['word'].size()[0])
...
class GPT2PAIRSentenceEncoder(nn.Module):

    def __init__(self, pretrain_path, max_length): 
        nn.Module.__init__(self)
        self.gpt2 = GPT2DoubleHeadsModel.from_pretrained(pretrain_path, num_labels=2)
        self.max_length = max_length
        self.tokenizer = GPT2Tokenizer.from_pretrained('gpt2')

    def forward(self, inputs):
        x = self.gpt2(inputs['word'], token_type_ids=inputs['seg'], attention_mask=inputs['mask'])[0]
        return x
